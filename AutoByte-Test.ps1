...
# Initialize Windows Forms properly - MUST be done before any form creation
...
function New-WriteOutputDelegate {
    ...
    return {
        ...
        if ($TextBox.InvokeRequired) {
            $TextBox.Invoke({ param($msg, $clr) & $script:WriteOutputDelegate $msg $clr }, $Message, $Color)
            return
        }
...
function Start-AutoByteProGUI {
    ...
    # Create the write output delegate for this specific textbox
    $script:WriteOutputDelegate = New-WriteOutputDelegate -TextBox $richTextBox

    # Write-Output function using the delegate
    function Write-Output {
        param([string]$Message, [string]$Color = "Black")
